...
def getperi(f,buff=[1,3],withinrvir=True,physical=True,getomegaperi=True,h=.7):
    ...
    for i in range(len(f[1].data)):
        ddist=(f[1].data.dhost[i,1:]-f[1].data.dhost[i,:-1])/h #dr/dt
        #print ddist
        ...
        for j in wmin:
            ...
            if np.mean(ddist[pre])>=0 and np.mean(ddist[post])<=0 and np.median(ddist[pre])>=0 and np.median(ddist[post])<=0:
                ...
                dxhostall=np.interp(xtime,f[1].data.time[i,all][::-1],f[1].data.dxhost[i,all][::-1]/h)
                dyhostall=np.interp(xtime,f[1].data.time[i,all][::-1],f[1].data.dyhost[i,all][::-1]/h)
                dzhostall=np.interp(xtime,f[1].data.time[i,all][::-1],f[1].data.dzhost[i,all][::-1]/h)
